...
public interface Loader {
  ...
  /**
   * Loads a configuration-related object of the supplied {@code type} and returns it.
   *
   * <p><strong>Note:</strong> The rules governing how it is determined whether any given configuration-related object
   * is "of the supplied {@code type}" are currently wholly undefined.</p>
   *
   ...
   * href="doc-files/terminology.html#determinate">determinate</a> value.</p>
   *
   ...
   *
   * @exception NoSuchObjectException if the invocation was sound but the requested object was <a
   * href="doc-files/terminology.html#absent">absent</a>
   *
   * @exception ConfigException if the invocation was sound but the object could not be loaded for any reason not
   * related to <a href="doc-files/terminology.html#absent">absence</a>
   *
   * @exception IllegalArgumentException if the suplied {@code type} was invalid for any reason
   *
  ...
  /**
   * <em>{@linkplain #bootstrap(ClassLoader) Bootstraps}</em> a {@link Loader} instance for subsequent usage using the
   ...
   *
   * <p>This method may or may not return a <a href="doc-files/terminology.html#determinate">determinate</a> value. See
   * {@link #bootstrap(ClassLoader)} for details.</p>
   *
   * <p>Except as possibly noted above, the observable behavior of this method is specified to be identical to that of
   * the {@link #bootstrap(ClassLoader)} method.</p>
   *
   * @return a {@link Loader}; never {@code null}
   *
   * @exception java.util.ServiceConfigurationError if bootstrapping failed because of a {@link
   * ServiceLoader#load(Class, ClassLoader)} or {@link ServiceLoader#findFirst()} problem
   *
  ...
  /**
   * <em>Bootstraps</em> a {@link Loader} instance for subsequent usage.
   *
   * <p>The bootstrap process proceeds as follows:</p>
   *
   * <ol>
   *
   * <li>A <em>primordial {@link Loader}</em> is located with observable effects equal to those resulting from executing
   * the following code:
   *
   ...
   *
   * <li>The {@link #load(Class)} method is invoked on the resulting {@link Loader} with {@link Loader Loader.class} as
   * its sole argument.
   *
   * <ul>
   *
   ...
   *
   * <p><strong>Note:</strong> The implementation of this method may change without notice between any two versions of
   * this specification.  The requirements described above, however, will be honored in any minor version of this
   * specification within a given major version.</p>
   *
   * @param classLoader the {@link ClassLoader} used to {@linkplain ServiceLoader#load(Class, ClassLoader) locate
   * service provider files}; may be {@code null} to indicate the system classloader (or bootstrap class loader) in
   * accordance with the contract of the {@link ServiceLoader#load(Class, ClassLoader)} method; often is the return
   * value of an invocation of {@link Thread#getContextClassLoader() Thread.currentThread().getContextClassLoader()}
   *
   * @return a {@link Loader}; never {@code null}
   *
   * @exception java.util.ServiceConfigurationError if bootstrapping failed because of a {@link
